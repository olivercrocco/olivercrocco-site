...
The idea is simple. You give the tool a manuscript's title, abstract, and a handful of keywords sorted by how central each one is to the paper. It then searches [OpenAlex](https://openalex.org), a vast open catalog of scholarship, for authors who have genuinely published on those topics in the journals where reviewers for this kind of work actually live. I started with the four flagship journals of the Academy of Human Resource Development, then widened the pool to ninety-seven journals across adult education, higher education, management, careers, international education, and industrial-organizational psychology, so a paper that bridges fields meets reviewers who do too.

The tool does a few things I care about as an associate editor. It rewards genuine expertise over sheer volume, so the field's most-published names stop crowding out the people who have actually written on your subject. It screens conflicts: anyone at a submitting author's institution drops away, and if I supply the authors' ORCID identifiers, so do their recent co-authors. It keeps a private list of people I have invited lately and quietly sets them aside, so I stop handing the same tired names another request. And it proposes a panel of nine candidates, ranked by fit and chosen to spread across countries, institutions, career stages, and methods. A paper built on an integrative review gets someone who knows integrative reviews. I also recently sharpened how it reads career stage. It now gauges seniority by how long someone has been publishing, so a scholar early in a productive run still reads as early-career even as the papers pile up. The panel aims for a real mix of early-career, mid-career, and senior reviewers, and it caps how many senior scholars it will propose, so the emerging voices I most want at the table actually reach it. In my testing the panel routinely lands across half a dozen countries and several career stages, and includes scholars I would never have thought to ask.

Finding a good name is only half the job. You still have to reach the person, and editorial systems ask for an email and a current affiliation. So the tool now assembles what is publicly available for each proposed reviewer: their current institution and, where a scholar has chosen to publish one, an email, pulled from ORCID, the scholarly identity registry. For everyone else it hands me ready-made search links, so tracking down an address takes a click or two. It looks only at the shortlist, and it queries only the reviewers' own public identifiers, never a word of the manuscript. The old scavenger hunt across faculty pages is mostly done before I open my inbox.
...
6. **Verify, then invite.** The tool also assembles a contacts sheet for the panel: each reviewer's affiliation, any public email, and search links for the rest, so the invitation step moves quickly. Affiliations in any database drift, so confirm a current email before you reach out. The tool proposes; you decide.
